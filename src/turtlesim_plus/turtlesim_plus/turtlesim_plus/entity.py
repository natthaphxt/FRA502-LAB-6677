...
class Pizza(GraphicsEntity):
    ...
    def render(self, screen):
        img = self.graphics
        size = np.min(
            [math.floor(img.get_size()[0] / 2), math.floor(img.get_size()[1] / 2)]
        )
        screen.blit(
            img,
            (self.pose[0] / 5.44 * 250 - size, 500 - self.pose[1] / 5.44 * 250 - size),
        )
...
class Turtle(PhysicsEntity, GraphicsEntity):
    ...
    def update(self, cmd_vel: List[float] = [0.0, 0.0], dt: float = 0.1):
        ...
        if abs(dtheta) < 0.00000001:
            ...
            V = np.array(
                [[q_dtheta[1], q_dtheta[0] - 1], [1 - q_dtheta[0], q_dtheta[1]]]
            )
...
class Scanner:
    def __init__(
        self, radius: float = 0.5, range: float = math.pi * 2, color=(255, 0, 0, 127)
    ):
        self.radius = radius
        self.range = range  #
        ...
        for entity in entity_list.values():
            if not entity is turtle:
                ...
                if distance <= self.radius and abs(angle) <= self.range / 2:
                    ...
                    scanner_data_array.append(
                        ScannerData(type=type(entity), angle=angle, distance=distance)
                    )
    ...
    def render(self, screen, pose):
        ...
        for i in range(
            int(math.degrees(-self.range / 2)), int(math.degrees(self.range / 2)) + 1
        ):
            rads = math.radians(i) + pose[2]
            vertices.append(
                (
                    (pose[0] + self.radius * math.cos(rads)) / 5.44 * 250,
                    500 - (pose[1] + self.radius * math.sin(rads)) / 5.44 * 250,
                )
            )
...
class TurtleEatInterface(PhysicsEntity, GraphicsEntity):
    ...
    def update(
        self, dt: float = 0.1, entity_list: Dict[str, Entity] = {}, *args, **kwargs
    ):
        edibles_data, self.edibles = self.eat_range.measure(
            pose=self.pose, turtle=self, entity_list=entity_list
        )
...
class TurtleScannerInterface(PhysicsEntity, GraphicsEntity):
    def __init__(
        self, turtle: Turtle, scanner_radius: float = 1.0
    ):  # Add scanner_radius parameter
        PhysicsEntity.__init__(self, name=turtle.name)
        self.turtle = turtle
        # Set the desired radius here - change 1.0 to whatever size you want
        self.scanner = Scanner(
            radius=scanner_radius, range=math.pi, color=(255, 0, 0, 127)
        )  # Changed range to pi for semicircle
    ...
    def update(
        ...
        self.scanner_output, detected_entity_list = self.scanner.measure(
            pose=self.pose, turtle=self, entity_list=entity_list
        )

    def set_scanner_radius(self, radius: float):
        """Change the scanner radius"""
        self.scanner.radius = radius

    def set_scanner_range(self, range_radians: float):
        """Change the scanner range (in radians)"""
        self.scanner.range = range_radians
...
class EntityManager:
    ...
    def add_entity(self, entity):
        if isinstance(entity, self.entity_type):
            ...
            raise TypeError(
                f"The entity {entity.name} must be a {self.entity_type.__name__}."
            )
...
class Simulator(EntityManager):
    def __init__(
        ...
        mouse_callback: Callable = lambda x: print("mouse is clicked"),
    ):
